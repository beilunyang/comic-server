...
class ComicSpider(scrapy.Spider):
    ...
    start_urls = ['http://v2.api.dmzj.com/classify/0/1/%d.json?channel=ios&version=2.1.9' %i for i in range(2420)]
    ...
    def parse_comic(self, response):
        ...
        item['origin_cover'] = urlparse(data['cover']).path
        item['origin'] = '动漫之家'
    ...
    def parse_chapter(self, response):
        ...
        item['origin_images'] = [urlparse(url).path for url in data['page_url']]
        yield item
